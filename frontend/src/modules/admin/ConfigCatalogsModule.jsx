                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="rounded-xl border border-amber-200 bg-amber-50/30 p-5 space-y-4">
                        <div className="flex items-center gap-2">
                            ...
                            <p className="text-[10px] font-800 text-amber-700 uppercase tracking-widest">Datos Bancarios Institucionales</p>
                        </div>
                        <p className="text-[11px] text-slate-500">Estos datos se usan para el pago de <strong>admisión</strong> y de <strong>matrícula</strong>. Se mostrarán a postulantes y estudiantes.</p>
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <Field label="Nombre del banco">
                                <Input className="h-9 rounded-xl border-slate-200 bg-white text-sm" value={settings.bank_name || ""} onChange={(e) => setSettings({ ...settings, bank_name: e.target.value })} placeholder="Banco de la Nación" />
                            </Field>
                            <Field label="N° de Cuenta">
                                <Input className="h-9 rounded-xl border-slate-200 bg-white text-sm font-mono" value={settings.bank_account || ""} onChange={(e) => setSettings({ ...settings, bank_account: e.target.value })} placeholder="Ej: 00-123-456789" />
                            </Field>
                            <Field label="Titular de la cuenta">
                                <Input className="h-9 rounded-xl border-slate-200 bg-white text-sm" value={settings.bank_holder || ""} onChange={(e) => setSettings({ ...settings, bank_holder: e.target.value })} placeholder="IESPP Gustavo Allende Llavería" />
                            </Field>
                        </div>
                    </div>
